"""
Custom loss function and corresponding gradients in the HitBoost model.
...
In this application, absolute value of label represents T in survival data, 
...
def hit_eval_loss(preds, dtrain):
    """
    ...
    for i in range(N):
        if y[i] > 0:
            ...
            L2_den += -np.sum(p)
            L2_num += -np.sum((p < gamma) * p * (gamma - p)**2)
    L2 = 0 if L2_den == 0 else L2_num / L2_den
    # L Computation (L = L1 + theta * L2)
    return "Loss", L1 + theta * L2

def hit_eval_ci(preds, dtrain):
    """
...
def hit_grads(preds, dtrain):
    ...
    for k in range(N):
        ...
        if y[k] > 0:
            ...
            # For den and num
            den += -np.sum(p)
            num += -np.sum((p < gamma) * p * (gamma - p)**2)
            # For g_s1 and h_s1 
            # i.e. the first-order and second-order gradients related to set s1
            g_s1[:t[k]] = np.sum((p < gamma) * (gamma - p) * (3*p - gamma))
            h_s1[:t[k]] = np.sum((p < gamma) * (4*gamma - 6*p))
        # For g_s2 and h_s2
        ...
        g_s2 = np.sum((p < gamma) * (gamma - p) * (gamma - 3*p) * mask_s2, axis=0)
        h_s2 = np.sum((p < gamma) * (4*gamma - 6*p) * mask_s2, axis=0)
    ...
    # L = L1 + theta * L2
    grad = L1_grad + theta * L2_grad
    hess = L1_hess + theta * L2_hess
